        ...
        try
        {
            ...
            if (request.getParameter("connecter") != null)
            {
                ...
                try
                {
                    ...
                    if (CentreSportifHelper.getBiblioInterro(session).getGestionParticipant().informationsConnexionValide(userId,
                            motDePasse))
                    {
                        session.setAttribute("userID", userId);
                        if(CentreSportifHelper.getBiblioInterro(session).getGestionParticipant().utilisateurEstAdministrateur(userId))
            ...
            else if (request.getParameter("inscrire") != null)
            {
                ...
                try
                {
                    ...
                    if (matricule == null || matricule.equals(""))
                        throw new IFT287Exception("Vous devez entrer un matricule!");
                    ...
                    GestionCentreSportif centreSportifUpdate = CentreSportifHelper.getBiblioUpdate(session);
                    synchronized (centreSportifUpdate)
                    {
                    	centreSportifUpdate.getGestionParticipant().inscrireParticipant(prenom,nom, motDePasse,matricule, acces);
                    }
